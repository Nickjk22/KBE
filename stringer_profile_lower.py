    ...
    def stringer_profiles(self):
        with open(self.airfoil_name, 'r') as f:
            ...
            upper_in_region = [(x, z) for x, z in upper_points
                               if self.stringer_position <= x <= self.stringer_position + self.stringer_thickness/self.chord]
            ...
            for x, z in reversed(upper_in_region):
                upper_profile.append(self.position.translate(
                    "x", x * self.chord,
                    "z", (z - self.stringer_thickness/(self.chord/self.thickness_factor)) * self.chord * self.thickness_factor))
