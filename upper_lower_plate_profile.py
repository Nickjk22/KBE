...
class UpperLowerPlateProfile(Polygon):
    ...
    def plate_profiles(self):
        with open(self.airfoil_name, 'r') as f:
            ...
            for x, z in reversed(upper_in_region):
                upper_profile.append(self.position.translate(
                    "x", x * self.chord,
                    "z", (z - self.front_spar_thickness/self.chord) * self.chord * self.thickness_factor))
    ...
    def upper_plate(self):
        return Polygon(
            points=self.plate_profiles['upper_profile'],
            color="red",
            transparency=0.5
        )
    ...
    def lower_plate(self):
        return Polygon(
            points=self.plate_profiles['lower_profile'],
            color="blue",
            transparency=0.5
        )
...
if __name__ == '__main__':
    ...
    obj = UpperLowerPlateProfile(label="Plate profiles")
    display(obj)
